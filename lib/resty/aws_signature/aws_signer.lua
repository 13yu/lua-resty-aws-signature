local signature_basic = require('resty.aws_signature.signature_basic')
local util = require('resty.aws_signature.util')
...
local function validate_headers(headers)
    ...
    for k, v in pairs(headers) do
        ...
        if type(v) ~= 'string' then
            return nil, 'InvalidArgument', string.format(
                    'header value: %s, is not a string', tostring(v))
        end
...
local function modify_request_headers(request, presign, request_date)
    local has_amz_date
    local has_amz_content_sha256_header
    ...
    if has_amz_date == true or presign ~= true then
    ...
    if has_amz_content_sha256_header == true then
...
function _M.add_auth_v4(self, request, opts)
    ...
    opts = opts or {}
    opts.presign = opts.presign == true
    ...
    local hashed_payload = modify_request_headers(request, opts.presign,
                                                  request_date)
