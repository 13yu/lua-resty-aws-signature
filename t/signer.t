...
--- config
    location /t {
        rewrite_by_lua '
            ...
            local auth_ctx, err, msg = signer:add_auth_v4(request, {presign = true})
            if err ~= nil then
                ngx.log(ngx.ERR, "failed to add auth: " .. err .. " " .. msg)
            end
...
=== TEST 8: when use presign, specify customed expire time
--- http_config eval: $::HttpConfig
--- config
    location /t {
        rewrite_by_lua '
            ...
            local auth_ctx, err, msg = signer:add_auth_v4(request, {presign = true, expires=1234})
            if err ~= nil then
                ngx.log(ngx.ERR, "failed to add auth: " .. err .. " " .. msg)
            end
